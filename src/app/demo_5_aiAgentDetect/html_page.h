...
<body>
    <div class="container">
        <div class="header">
            <h1>🎥 人体检测视频流 - FLV流模式</h1>
            <p>YOLOv5实时检测 • 检测框绘制 • 硬件H.264编码</p>
        </div>
                ...
                <div class="video-section">
                    ...
                    <div class="video-container" id="videoContainer">
                        ...
                        <div id="videoPlaceholder" class="video-placeholder">
                            ...
                            <h3 style="margin-bottom: 10px;">等待 PicoClaw 启动检测</h3>
                            <p>通过 PicoClaw 语音命令启动视频流</p>
                            <p style="margin-top: 15px; font-size: 14px; color: #adb5bd;">
                                例如: "启动人体检测，监控 rtsp://192.168.1.100/stream"
                            </p>
                        </div>
                    ...
                    <div class="mosaic-controls">
                        <div class="toggle-container">
                            <span class="toggle-label">
                                <span class="material-icons-outlined">border_outer</span>
                                检测框绘制
                            </span>
                            <label class="toggle-switch">
                                <input type="checkbox" id="bboxEnabled" checked>
                                <span class="toggle-slider"></span>
                            </label>
                            <span id="bboxStatus" style="font-size: 14px; color: #28a745;">
                    ...
                    <div class="status-container">
                        <div class="status-box">
                            ...
                            <div id="detectionStatus" style="margin-top: 10px; font-size: 14px;">
                                <span class="status-indicator" style="margin-right: 5px;"></span>
                                <span id="detectedCount">未检测</span>
                            </div>
                            <div id="statusText">未连接</div>
                            <div class="status-indicator" id="statusIndicator"></div>
                            <div id="currentStream" class="stream-url">
                                <strong>当前流:</strong> <span id="streamUrl">等待 PicoClaw 启动</span>
                            </div>
                        </div>
                    </div>

                    <div class="compatibility-notice">
                        <strong>提示：</strong> 您可以通过 PicoClaw 语音命令启动检测，页面会自动开始播放视频。
                    </div>
                </div>
            ...
            <div class="right-panel">
                <div class="perf-board">
                    ...
                    <div class="perf-flow">
                        ...
                        <div class="perf-node">
                            <div class="perf-label">
                                <span class="material-icons-outlined" style="color: #4b6cb7;">border_outer</span>
                                检测框 (BBox)
                            </div>
    ...
    <script>
        ...
        // 应用检测框设置
        function applyBboxSettings() {
            const enabled = document.getElementById('bboxEnabled').checked;

            fetch('/update_bbox_settings', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                },
                body: 'enabled=' + enabled + '&thickness=3'
            })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    const statusElement = document.getElementById('bboxStatus');
                    if (enabled) {
            ...
            .catch(error => {
                console.error('Failed to update bbox settings:', error);
            });
        }
        ...
        async function updateDetectionStatus() {
            ...
            try {
                const response = await fetch('/get_detection_result');
                const data = await response.json();
                const detectedCount = document.getElementById('detectedCount');
                const detectedIndicator = detectedCount.querySelector('.status-indicator');

                if (data.detected && data.count > 0) {
                    detectedCount.innerHTML = '<span class="status-indicator active" style="margin-right: 5px;"></span>' +
                        `检测到 ${data.count} 人 (置信度: ${data.confidence.toFixed(2)})`;
                    if (detectedIndicator) {
                        detectedIndicator.classList.add('active');
                    }
                } else {
                    detectedCount.innerHTML = '<span class="status-indicator" style="margin-right: 5px;"></span>未检测到人员';
                    if (detectedIndicator) {
                        detectedIndicator.classList.remove('active');
                    }
                }
            } catch (error) {
        ...
        // 检测框开关事件
        document.getElementById('bboxEnabled').addEventListener('change', applyBboxSettings);
        ...
        function startStreamStatusPolling() {
            setInterval(checkStreamStatus, 2000); // 每2秒检查一次
            setInterval(updateDetectionStatus, 1000); // 每秒更新检测状态
        }
        ...
        async function checkStreamStatus() {
            try {
                ...
                const currentlyStreaming = data.is_streaming;
                ...
                if (currentlyStreaming !== isStreaming) {
                    if (currentlyStreaming) {
                        // PicoClaw启动了流，自动开始播放
                        console.log('检测到新流，自动启动播放器:', currentUrl);
                        document.getElementById('rtspUrl').value = currentUrl;
                        currentStreamUrl = currentUrl;
                        isStreaming = true;
                        updateStatus('检测到视频流，正在启动播放...', true);
                        showLoading(true);
                ...
                } else if (isStreaming && currentUrl && currentUrl !== currentStreamUrl) {
                    // RTSP URL发生了变化，需要重新连接
                    console.log('RTSP URL发生变化，重新连接');
                    stopFLVPlayer();
                    currentStreamUrl = currentUrl;
                    isStreaming = true;
