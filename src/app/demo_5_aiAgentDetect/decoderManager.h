...
public:
    ...
    // 保存检测到的最新JPEG图片
    bool save_detected_jpeg(const std::string& output_dir = "/userdata/tmp/picoclaw/workspace/detect_images");
    std::string get_latest_jpeg_path() const;
    bool get_latest_jpeg_data(std::vector<uint8_t>& data) const;
    void enable_auto_save_jpeg(bool enable, int min_interval_seconds = 30);

    int  updateDetectionByDetect(const emai::YUVFrame& frame, double& detect_time_ms);
    ...
    struct DetectionResult {
        ...
        std::chrono::high_resolution_clock::time_point detect_time;
    };
    DetectionResult get_detection_result();
    std::string get_detection_result_json();

    // 更新检测框绘制设置
    void update_bbox_settings(bool enabled, int line_thickness = 3);
...
private:
    ...
    // 检测框绘制器
    emai::BBoxDrawer::Ptr                             bboxDrawer_{nullptr};

    // 检测结果存储
    std::mutex                                        detectResultMutex_;

    emai::BOX_RECT                                    curDetectedBox_{0, 0, 0, 0};
    float                                             curDetectedConfidence_ = 0.0f;
    std::chrono::high_resolution_clock::time_point    curDetectTime_;

    emai::BOX_RECT                                    lastDetectedBox_{0, 0, 0, 0};
    float                                             lastDetectedConfidence_ = 0.0f;
    std::chrono::high_resolution_clock::time_point    lastDetectTime_;
    int64_t                                           lastDetectedCnt{0};
    ...
    bool                                              enable_bbox_draw_{true};
    int                                               bbox_line_thickness_{3};
    std::chrono::high_resolution_clock::time_point    onDecodeFrameTime_;

    // JPEG 编码器（用于保存检测到的图片）
    emai::IJpegEncoder::Ptr                          jpegEncoder_{nullptr};
    std::vector<uint8_t>                              latestJpegData_;
    mutable std::mutex                                jpegMutex_;
    std::string                                       latestJpegPath_;
    std::chrono::high_resolution_clock::time_point    lastJpegSaveTime_;
    bool                                              enableAutoSaveJpeg_{false};
    int                                               minJpegSaveIntervalSeconds_{30};  // 最小保存间隔（秒）
    std::string                                       jpegSaveDir_{"/userdata/tmp/picoclaw/workspace/detect_images"};
};