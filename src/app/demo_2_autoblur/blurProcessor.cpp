...
std::vector<uint8_t> BlurProcessor::process_and_encode(emai::YUVFrame& input_yuv) 
{
    ...
    try {
        ...
        if (!output_yuv.empty()) {
            ...
            LOG_DEBUG("Encoding frame done, time: " << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count() << " ms");
        }
...
void BlurProcessor::update_blur_settings(const BlurSettings &blurSettings) 
{
    std::lock_guard<std::mutex> lock(processor_mutex);
    blur_settings = blurSettings;
    
    // 重新生成高斯模糊内核
    generate_gaussian_kernel(blur_settings.blur_radius);    
...
void BlurProcessor::apply_blur_to_frame() 
{
    ...
    if (blur_settings.shape == "circle") {
        LOG_ERROR("Circular blur is not implemented yet");
    } else {
...
void BlurProcessor::add_border_to_region() 
{
    ...
    if (blur_settings.shape == "circle") {
        LOG_ERROR("Circular border is not implemented yet");
    } else {
        // 矩形边框
...
void BlurProcessor::getDstWidthHeight(int& width, int& height)
{
    width = dst_width;
    height = dst_height;
}
